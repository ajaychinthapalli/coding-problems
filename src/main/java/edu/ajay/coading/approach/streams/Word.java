...
public class Word {
	...
	protected String reverseWordUsingStreams(String employeeName) {
		return PATTERN.splitAsStream(employeeName).map(word -> new StringBuilder(word).reverse())
	...
	protected Boolean containsOnlyDigits(String employeeName) {
		// Notice that Java 8 functional style and regular expression-based solutions
		// are usually slow,
		// so if performance is a requirement, then it's better to rely on the
		// Character.isDigit() approach.

		// Avoid solving this problem using parseInt() or parseLong().
		// First of all, It's bad practice to catch NumberFormatException and take
		// business logic decisions in the catch block.
		// Second, these methods verify whether the string is a valid number, not
		// whether it contains only digits( for example -4 is valid).
		// For third-party library support, please consider the Apache Commons Lang,
		// StringUtils.isNumeric().

		// In Java 8 functional style, the normal style Character.isDigit() code can be
		// rewritten using anyMatch()
		return employeeName.chars().anyMatch(digit -> isDigit(digit));
	}

	/**
	 * Counting vowels and consonants. In Java 8 functional style, the code can be
	 * written using chars() and filter()
	 *
	 * @param employeeName
	 * @return Pair
	 */
	...
	/**
	 * Counting vowels and consonants. The given string employeeName is filtered
	 * accordingly and the count() terminal operation returns the result. Relying on
	 * partitioningBy() will reduce the code.
	 *
	 * @param employeeName
	 * @return Pair
	 */
	...
	/**
	 * Counting the occurrences of a certain character. In Java 8 functional style,
	 * the code can be written using chars() and filter().
	 *
	 * Other ways to implement this using third-party library support: -> Apache
	 * Commons Lang: StringUtils.countMatches() -> Spring Framework:
	 * StringUtils.countOccurrencesOf() -> Guava: CharMatcher.is().countIn()
	 *
	 * @param employeeName
	 * @param character
	 * @return Integer
	...
	protected Integer countOccurrencesOfACertainCharacter(String employeeName, Character character) {
		Long occurrencesCount = employeeName.chars().filter(ch -> ch == character).count();
		return occurrencesCount.intValue();
	}

	/**
	 * Joining multiple strings with a delimiter. In Java 8, one of the solution
	 * relies on the StringJoiner. It supports an optional prefix and suffix as well
	 * (ignored here)
	 *
	 * @param delimiter
	 * @param args
	...
	/**
	 * Joining multiple strings with a delimiter. Another solution relies on the
	 * String.join() method. This method was introduced in Java 8 comes in two
	 * flavours. -> String join(CharSequence delimiter, CharSequence... elems) ->
	 ...
	 * @return String
	 */
	protected String joinByDelimiterUsingJoin(String delimiter, String firstName, String lastName) {
		return join(delimiter, firstName, lastName);
	}

	/**
	 * Joining multiple strings with a delimiter. Another solution relies on streams
	 * and Collectors.joining()
	 *
	 * For third-party library support, please consider -> Apache Commons Lang:
	 * StringUtils.join() -> Guava: Joiner
	 *
	 * @param delimiter
	 * @param args
	 * @return String
	...
	protected String joinByDelimiterUsingStreams(Character delimiter, String... args) {
		return stream(args, START_INCLUSIVE, args.length).collect(joining(valueOf(delimiter)));
	}
	...
	protected void permuteAndPrintStream(String employeeName) {
		...
		// prefix will
		// concatenate (fix) the next letter from the string. The remaining letters are
		// passed
